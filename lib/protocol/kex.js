...
const createKeyExchange = (() => {
  ...
  //TODO: Prime generation takes 30-60 seconds. Look into generating at startup or using pre-determined p and g values
  const generateDHParams = (minBits, prefBits, maxBits) => {
    // Creating DiffieHellman 
    console.log("Creating Diffie Hellman. This may take a while...")
    const k = createDiffieHellman(prefBits);
        
    // Accessing prime value
    const secure_prime = Buffer.from(k.getPrime());
    // Accessing generator value
    const generator = Buffer.from(k.getGenerator());

    console.log("Prime generated!")
    console.log(secure_prime)
    //returns array [(Buffer) Secure Prime, (Buffer) Generator] 
    return [secure_prime, generator]
  }

  //High Speed Solution using RFC3526 Prime and Generator inputs to create diffie hellman object  
  //https://www.rfc-editor.org/rfc/rfc3526#section-3 
  const generateDHParamsRFC3526 = (minBits, prefBits, maxBits) => {
    /*
    2048-bit MODP Group
    Prime:
    ...
    */  
    ...
    if(prefBits!==2048){
      return doFatalError(
        this._protocol,
        'Requested prime size unsupported',
        'handshake',
        DISCONNECT_REASON.KEY_EXCHANGE_FAILED
      );
    }

    var primeArray = [0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xC9, 0x0F, 0xDA, 0xA2, 0x21, 0x68, 0xC2, 0x34, 0xC4, 0xC6, 0x62, 0x8B, 0x80, 0xDC, 0x1C, 0xD1,
    ...
    const secure_prime = Buffer.from(primeArray);
    const generator = 2;
    const k = createDiffieHellman(secure_prime, generator);
    const p = Buffer.from(k.getPrime());
    const g = Buffer.from(k.getGenerator());
      ...
      switch (this._step) {
        case 1:
          ...
          */
          ...
          this._protocol._getDHParams = generateDHParamsRFC3526;
